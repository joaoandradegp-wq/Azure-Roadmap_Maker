...
// Tag de duração em sprints: "2 sprints", "1 Sprint", "3 SPRINTS" etc — qualquer número,
// singular ou plural, caixa alta ou baixa. Controla quantas sprints (contando a atual
// como a primeira) o card deve levar até a entrega, estendendo a sprint de referência
// usada pra calcular a previsão. Não muda a regra de "próxima terça/quinta após o fim
// da sprint (ou depois do freeze)" — só troca QUAL sprint entra como referência de fim.
...
/**
 * Extrai a duração em sprints a partir das tags do item (ex: "2 Sprints" -> 2).
 * Se por engano houver mais de uma tag desse tipo, usa a maior. Retorna null se
 * não houver nenhuma (comportamento padrão: 1 sprint, sem estender nada).
 */
 ...
 * quinta), assume que foi entregue nesse dia anterior — só corrige 1 dia pra
 * trás. Fora esse caso, usa a própria data de fechamento como a entrega real
 * (e NÃO empurra pra frente até a próxima terça/quinta "oficial" — publicação
 * emergencial fora do ciclo normal pode acontecer em qualquer dia, então
 * forçar pra grade de terça/quinta apagaria a data real de uma entrega
 * adiantada, ou pior, faria ela parecer mais tarde do que realmente foi).
 */
...
function transform(workItems, config, { today = new Date() } = {}) {
  ...
  for (const item of workItems) {
    ...
    // ---- Tag de duração ("2 sprints" etc): estende a sprint de referência ----
    // Conta a sprint atual do card como a primeira, então "2 sprints" usa o fim
    // da PRÓXIMA sprint como referência (sprintNumber + 1), "3 sprints" usa
    // sprintNumber + 2, e assim por diante. Só afeta o FIM usado pra calcular a
    // previsão de entrega — o início da barra continua sendo o da sprint atual.
    ...
    // Bloqueado (essas tags já mapeiam para phaseStatus "pausado" — bolinha
    // laranja — no statusMapping.byTag do config.json; são uma decisão/situação
    // deliberada, não um atraso "silencioso", então não fazem sentido serem
    // escondidas atrás da mesma bolinha vermelha genérica de "não entregue
    // na sprint").
    ...
    if (!isClosed) {
      ...
      // Riscado + nova data ao lado sempre que a entrega real caiu num dia
      // DIFERENTE do que era esperado (pra frente = atraso, ou pra trás =
      // publicação emergencial adiantada) — não só quando atrasou. Se
      // entregou exatamente no dia previsto, mostra só a data, sem riscado.
      ...
      if (plannedDeployDate && actualDeployDate && !deliveredOnPlannedDate) {
        plannedDateOut = formatDateBR(plannedDeployDate); // data que era esperada (riscada)
        actualDateOut = formatDateBR(actualDeployDate); // data real de entrega
      } else {
        actualDateOut = formatDateBR(actualDeployDate); // deploy real, no dia previsto
      }
    }
