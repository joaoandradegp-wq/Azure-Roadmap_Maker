 ...
 * Se a própria data já for terça/quinta, retorna ela mesma.
 */
...
function transform(workItems, config, { today = new Date() } = {}) {
  ...
  for (const item of workItems) {
    ...
    // "Previsão" (ainda não fechado) e "Concluída no prazo": mostra só a data real
    // de deploy, sem tracinho. "Concluída com atraso": mostra a data de deploy
    // ORIGINAL (da sprint prevista) riscada, seguida da data de deploy REAL.
    ...
    if (!isClosed) {
      // Previsão nunca mostra uma data que já passou: se o deploy previsto (fim
      // da sprint) já ficou pra trás e o item ainda está aberto, empurra a
      // previsão pro próximo dia de deploy a partir de hoje.
      const forecastDate =
        plannedDeployDate && plannedDeployDate >= todayMidnight
          ? plannedDeployDate
          : nextDeployDate(todayMidnight);
